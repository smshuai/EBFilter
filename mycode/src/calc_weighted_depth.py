...
def process_args():
    ...
    if len(sys.argv) == 7:
        bam = sys.argv[1]
        mstat = pd.read_csv(sys.argv[2], index_col='name')
        umap = np.loadtxt(sys.argv[3], np.str, comments=None)
        U1U11 = sys.argv[4]
        donor_id = sys.argv[5]
        out_path = sys.argv[6]
    else:
        sys.stderr.write('Usage: calc_weighted_depth.py BAM map_stat umapper U1_bed ID OUT\n')
        sys.stderr.write('\t- BAM: Bowtie2 re-aligned and post-processed BAM file\n')
        sys.stderr.write('\t- map_stat: Mapping statistics from post-processing step\n')
        sys.stderr.write('\t- umapper: Names of unique mapping reads from post-processing step\n')
        ...
        sys.exit(1)
    return bam, mstat, umap, U1U11, donor_id, out_path
...
if __name__ == '__main__':
    # Get arguments
    bam, mstat, umap, U1U11, donor_id, out_path = process_args()
    # Make three pileup files (/1, /2, unpair)
    cmd1 = 'samtools mpileup -l {} --rf 64 --ff UNMAP,QCFAIL,DUP -a --output-QNAME -Q 15 {} > {}'.format(U1U11, bam, bam + '_1.pileup')
    cmd2 = 'samtools mpileup -l {} --rf 128 --ff UNMAP,QCFAIL,DUP -a --output-QNAME -Q 15 {} > {}'.format(U1U11, bam, bam + '_2.pileup')
    cmd3 = 'samtools mpileup -l {} --ff 1,UNMAP,QCFAIL,DUP -a --output-QNAME -Q 15 {} > {}'.format(U1U11, bam, bam + '_3.pileup')
    run_subprocess(cmd1)
    run_subprocess(cmd2)
    run_subprocess(cmd3)
    results = dict()
